...
public class Greenfoot
{
    ...
    /**
     * Repaint the world and wait for a while according to the simulation speed
     * Will pause indefinitely if Simulation's hold variable is set to true
     */
    public static void hold()
    {
		...
		while (true)
		{
			// Sleep indefinitely until the simulation status is not hold
			// or if the continue once status is true
			simulation.sleep();
			if (simulation.isContinueOnce())
			{
				simulation.setContinueOnce(false);
				break;
			} 
			else
			if (!simulation.isHolded())
			{
				break;
			} 
		}
