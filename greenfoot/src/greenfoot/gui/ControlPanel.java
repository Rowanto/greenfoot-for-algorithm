...
import greenfoot.actions.HoldSimulationAction;
...
import greenfoot.actions.UnholdSimulationAction;
    ...
    implements ChangeListener, SimulationListener
{
    ...
    private HoldSimulationAction holdSimulationAction;
    private UnholdSimulationAction unholdSimulationAction;
    ...
    private JPanel createButtonPanel(Simulation simulation, boolean includeAllControls)
    {
        ...
        holdSimulationAction = HoldSimulationAction.getInstance();
        holdSimulationAction.attachSimulation(simulation);
        holdSimulationAction.putValue(Action.LONG_DESCRIPTION, 
                Config.getString("controls.hold.longDescription"));
        holdSimulationAction.putValue(Action.SHORT_DESCRIPTION, 
                Config.getString("controls.hold.shortDescription"));
        holdSimulationAction.setEnabled(false);        
        holdButton = GreenfootUtil.createButton(holdSimulationAction);
        
        unholdSimulationAction = UnholdSimulationAction.getInstance();
        unholdSimulationAction.attachSimulation(simulation);
        unholdSimulationAction.putValue(Action.LONG_DESCRIPTION, 
                Config.getString("controls.unhold.longDescription"));
        unholdSimulationAction.putValue(Action.SHORT_DESCRIPTION, 
                Config.getString("controls.unhold.shortDescription"));
        unholdSimulationAction.setEnabled(false);        
        unholdButton = GreenfootUtil.createButton(unholdSimulationAction);        
        ...
        if(holdButton.getPreferredSize().getWidth() > unholdButton.getPreferredSize().getWidth()) {
        	unholdButton.setPreferredSize(holdButton.getPreferredSize());
        	unholdButton.setMaximumSize(holdButton.getMaximumSize());
        	unholdButton.setMinimumSize(holdButton.getMinimumSize());
        } else  {
        	holdButton.setPreferredSize(unholdButton.getPreferredSize());
        	holdButton.setMaximumSize(unholdButton.getMaximumSize());
        	holdButton.setMinimumSize(unholdButton.getMinimumSize());
        }        
    ...
    {        
        ...
        SwingUtilities.invokeLater(new Thread() {
            public void run()
            {
                ...
                else if (etype == SimulationEvent.HOLDED) {
                    if (speedSlider != null) {
                        speedSlider.setEnabled(true);
                    }
                    buttonPanel.remove(holdButton);
                    buttonPanel.add(unholdButton, 2);
                    buttonPanel.validate();
                }
                else if (etype == SimulationEvent.CONTINUED) {
                    if (speedSlider != null) {
                        speedSlider.setEnabled(true);
                    }
                    buttonPanel.remove(unholdButton);
                    buttonPanel.add(holdButton, 2);
                    buttonPanel.validate();
                }                
